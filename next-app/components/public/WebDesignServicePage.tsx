...
function WebDesignMockupCtaBand({
  ...
  return (
    <section className={cn("border-b border-slate-200/50", bgClass)}>
      <div className={cn(shell, sectionYRelaxed)}>
        <div
          className={cn(
            mmsCard,
            "mx-auto max-w-3xl border-amber-200/30 bg-gradient-to-br from-white via-white to-amber-50/35 p-8 sm:p-10",
          )}
        >
...
export function WebDesignServicePage() {
  return (
    <div className={mmsPageBg}>
      {/* 1. Hero */}
      <section className="relative overflow-hidden border-b border-slate-200/70 bg-gradient-to-b from-white via-[#faf9f6] to-[#ebe8e2]/90 shadow-[inset_0_-1px_0_rgba(15,23,42,0.04)]">
        <div
          className="pointer-events-none absolute inset-0 opacity-100"
          style={{
            backgroundImage:
              "radial-gradient(ellipse 85% 55% at 15% -10%, rgba(234, 88, 12, 0.08), transparent 55%), radial-gradient(ellipse 60% 45% at 95% 15%, rgba(15, 118, 110, 0.06), transparent 50%), linear-gradient(180deg, rgba(255,255,255,0.55) 0%, transparent 42%)",
          }}
          aria-hidden
        />
        <div className={cn(shell, heroSectionY, "relative z-[1] max-w-4xl")}>
          ...
          <p className="mt-8 max-w-xl text-sm font-medium text-slate-600 md:text-[0.9375rem]">
      ...
      {/* 2. Who this is for */}
      <section className="border-b border-slate-200/50 bg-white/90">
        <div className={cn(shell, sectionYRelaxed, "max-w-3xl")}>
          <h2 className={mmsH2}>Who this is for</h2>
          <p className="mt-8 max-w-2xl text-base leading-relaxed text-slate-600 md:text-lg">
      ...
      {/* 3. What you get */}
      <section className="border-b border-slate-200/50 bg-[#f4f3ef]/85" id="browse-by-type">
        <div className={cn(shell, sectionYRelaxed)}>
      ...
      {/* 4. What happens next */}
      <section className="border-b border-slate-200/50 bg-white/92">
        <div className={cn(shell, sectionYRelaxed)}>
          ...
          <div className="mt-14 grid gap-8 md:grid-cols-3 md:gap-10">
            {[
              ...
              <div key={step.n} className={cn(mmsCard, "p-8 sm:p-9")}>
                <span
                  className="mb-5 inline-flex h-10 w-10 items-center justify-center rounded-full bg-amber-100 text-sm font-bold text-amber-900"
                  aria-hidden
                >
                  {step.n}
                </span>
                <h3 className="text-lg font-bold leading-snug text-slate-900">{step.title}</h3>
                <p className="mt-4 text-sm leading-relaxed text-slate-600 md:text-[15px]">{step.line}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="border-b border-slate-200/50 bg-[#f4f3ef]/88">
        <div className={cn(shell, sectionYRelaxed, "max-w-3xl")}>
      ...
      <section className="border-b border-slate-200/55 bg-white/80 py-14 md:py-16">
        <div className={cn(shell, "max-w-2xl text-center")}>
          ...
          <p className="mt-4 text-base text-slate-600 md:text-[17px]">
      ...
      {/* 8. Final CTA */}
      <section className="bg-gradient-to-b from-[#ece8e0]/95 to-[#e8edf2]/95">
