...
export async function GET(
  ...
  console.info("[Scout Proxy] session found for outreach thread:", hasSession);
  console.info("[Scout Proxy] token found for outreach thread:", hasAccessToken);
  if (!accessToken) {
    return NextResponse.json(
  ...
  console.info("[Scout Proxy] forwarding Authorization header for outreach thread");
  ...
  try {
    ...
    const response = await fetch(
      `${baseUrl}/outreach/thread/${encodeURIComponent(params.leadId)}`,
      {
        method: "GET",
        headers,
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Outreach thread request failed.",
      },
      { status: 502 }
    );
  }
