...
export async function POST(request: Request) {
  ...
  console.info("[Scout Proxy] session found for outreach generate:", hasSession);
  console.info("[Scout Proxy] token found for outreach generate:", hasAccessToken);
  console.info("[Scout Proxy] token source for outreach generate:", tokenSource);
  if (!accessToken) {
    return NextResponse.json(
  ...
  console.info("[Scout Proxy] forwarding Authorization header for outreach generate");
  if (workspaceId) headers["X-Workspace-Id"] = workspaceId;

  try {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), DEFAULT_TIMEOUT_MS);
    const response = await fetch(`${baseUrl}/outreach/generate-email`, {
      method: "POST",
      headers,
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Outreach generate request failed.",
      },
      { status: 502 }
    );
  }
